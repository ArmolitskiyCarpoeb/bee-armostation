	...
	if(free)
		...
		if(D)
			if(!transfer_money(D, money_to_transfer))
				bank_card_talk("Счёт департамента не имеет достаточно средств для заработной платы.")
				return FALSE
			else
				bank_card_talk("На ваш счёт поступило $[account_balance].")
				//The bonus only resets once it goes through.
				if(paycheck_bonus > 0) //And we're not getting rid of debt
					paycheck_bonus = 0
				return TRUE
	bank_card_talk("Невозможно найти счёт департамента для заработной платы.")
	return FALSE
